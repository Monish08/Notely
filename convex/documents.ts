...
export const getTrash = query({
    handler: async (ctx) => {
        ...
        const userId = identity.subject;
        const documents = await ctx.db
            .query("documents").withIndex("by_user",(q)=>q.eq("userId",userId)).filter((q) => q.eq(q.field("isArchived"), true)).order("desc").collect();
        return documents;
    }
})
...
export const getById = query({
  ...
  handler: async (ctx, args) => {
    ...
    // ✅ Public access if published & not archived
...
export const removeCoverImage = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (existingDocument.userId !== userId) {
      throw new Error("Not authorized to update this Cover Image");
    }
    const document = await ctx.db.patch(args.id, {
